...
func main() {
	...
	fmt.Printf("Loaded infrastructure model version %s\n", model.Metadata.Version)
	fmt.Printf("Environment: %s\n", model.Metadata.Environment)
	fmt.Printf("Total devices: %d\n\n", len(model.Devices))

	device, err := GetDeviceByID(model, "rtr-core-01")
	if err != nil {
	...
	err = UpdateDeviceInterface(model, "rtr-core-01", "GigabitEthernet0/0/0", updates)
	if err != nil {
	...
	err = AddStaticRoute(model, "rtr-core-01", newRoute)
	if err != nil {
	...
	err = UpdateDeviceManagementIP(model, "sw-access-01", "10.0.1.25")
	if err != nil {
	...
	err = SaveModel("infrastructure_updated.json", model)
	if err != nil {
		fmt.Printf("Error saving model: %v\n", err)
		return
	}
	fmt.Println("\nModel saved successfully to infrastructure_updated.json")
}